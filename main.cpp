...
void Sunflower::produceSun() {
	timer++;
	if (timer > 600) {	// 每600帧（约10秒）生产一个阳光
		timer = 0;
		// 找一个未使用的阳光对象
		int k;
		for (k = 0; k < ballMax && balls[k].used; k++);
		...
		balls[k].used = true;
		balls[k].p1 = vector2(x, y);
		int w = (100 + rand() % 50) * (rand() % 2 ? 1 : -1);	// 左右随机偏移
		balls[k].p4 = vector2(x + w, y + 50);
		balls[k].p2 = vector2(balls[k].p1.x + w * 0.3, balls[k].p1.y - 200);
		balls[k].p3 = vector2(balls[k].p1.x + w * 0.7, balls[k].p1.y - 200);
		balls[k].status = SUNSHINE_PRODUCT;
		balls[k].speed = 0.02;
	...
	static int zmFre = 900;  // 僵尸生成频率（帧）
	static int count = 0;    // 帧计数器
	count++;
	if (count > zmFre) {
		count = 0;
		zmFre = 1000 + rand() % 400; // 下一次生成时间（1000~1400帧随机）
